...
import com.prabhix.mailroom.data.api.ApiErrorBody
...
class AuthInterceptor @Inject constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        session?.let {
            ...
            // Always the session's own organization, and never overridden. Mailroom shows the mail of
            // whoever is signed in; there is no impersonation here and nothing that reads across
            // tenants, so a caller-supplied scope would only be a way to get it wrong.
 ...
 * needs the Retrofit instance built on the very OkHttpClient this authenticator is installed into.
 * Resolution is deferred to the first 401, by which point the graph is fully built.
 */
@Singleton
