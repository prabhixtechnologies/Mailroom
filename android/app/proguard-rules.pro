...
# cannot see statically. A missing rule does not fail the build — it produces an APK that installs
# and then crashes on first use, so treat changes here as release-blocking.
...
-keep class com.prabhix.mailroom.data.api.** { *; }
...
# ---------------------------------------------------------------------------
# Tink carries Error Prone's compile-time annotations, which are not on the runtime classpath.
# Annotations are erased at runtime, so there is nothing to keep here — only the warning to silence.
