 ...
 *
 * <p>Mailroom has no password form and never will. It has no user table, no credential to check and no
 * session of its own to establish: it redirects to the hosted login page on Identity's origin, and the
 * cookie set there is what lets somebody who signed in to OneOps this morning open their mail without
 * being asked again. That is what single sign-on means. Three apps with three login forms is three
 * logins and three places a password can be stolen from.
 *
 ...
 * this origin, and is what proves the app redeeming the code is the one that asked for it.
 */

/** Matches the client id seeded by Identity's RegisteredClientSeeder. */
const CLIENT_ID = "prabhix-mailroom";

// sessionStorage, not localStorage. The verifier is single-use and worthless after the exchange, and a
// per-tab scope means two sign-in attempts in two tabs cannot overwrite each other's.
...
export async function beginLogin(returnTo?: string): Promise<void> {
  ...
  const params = new URLSearchParams({
    ...
    // the authorization request can also redeem the code — the attack PKCE exists to stop.
