...
import { helpdeskKeys } from "./helpdesk";
...
export function useCreateMailbox() {
  const invalidate = useMailboxInvalidation();
  return useMutation({
    mutationFn: (vars: { address: string; name: string; description?: string }) =>
      apiRequest("/mail/mailboxes", mailboxAdminSummarySchema, {
        body: { address: vars.address, name: vars.name, description: vars.description, kind: "SHARED" },
      }),
