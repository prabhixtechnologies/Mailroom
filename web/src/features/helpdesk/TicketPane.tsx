...
export function TicketPane({
  ...
  const tags = useTags();
  const members = useAssignableMembers(me?.organizationId);
  ...
  if (detail.isError || !ticket) {
    return (
      <EmptyState
        ...
        hint={detail.error ? getApiErrorMessage(detail.error) : undefined}
      />
    );
  }
            ...
            return (
              ...
              >
                <div className="mb-2 flex items-center gap-2 text-xs text-text-muted">
                  <Avatar label={initials(m.fromName || m.fromAddress || "?")} />
                  {outbound ? (
