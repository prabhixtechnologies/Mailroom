...
import { useMailboxDetail } from "@/lib/mailbox-admin";
import { cn } from "@/lib/utils";
import { adminErrorHint } from "./SettingsLayout";

export function MailboxDetailLayout() {
  ...
  if (detail.isError || !detail.data) {
    return (
      <EmptyState
        ...
        hint={adminErrorHint(detail.error) ?? getApiErrorMessage(detail.error)}
      />
    );
  }

  const mb = detail.data;
  const base = `/settings/mailboxes/${mailboxId}`;
  const tabs = [
