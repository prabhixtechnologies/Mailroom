    ...
    <div className="mx-auto max-w-3xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-text-muted">
            Organization-wide labels applied to tickets in the queue.
          </p>
        </div>
      ...
      {tags.isPending ? (
        ...
        <EmptyState
          ...
          hint={adminErrorHint(tags.error) ?? getApiErrorMessage(tags.error)}
        />
      ) : (tags.data ?? []).length === 0 ? (
        <EmptyState
          icon={<TagIcon className="size-8" />}
          title="No tags yet"
          hint="Create tags to classify tickets in the queue."
        />
      ) : (
