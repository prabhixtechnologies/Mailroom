...
import { useAssignableMembers } from "@/lib/helpdesk";
import {
  memberAccessLevels,
  useAddMailboxMember,
...
export function MailboxMembersSection() {
  ...
  const { me } = useAuth();
  const orgMembers = useAssignableMembers(me?.organizationId);
    ...
    <div>
      ...
      {mailbox.members.length === 0 ? (
        <EmptyState
          ...
          hint="Add users or teams who should see this mailbox in the queue."
        />
      ) : (
        <ul className="divide-y divide-border rounded-lg border border-border">
          {mailbox.members.map((member) => (
            <li key={member.id} className="flex items-center gap-3 px-4 py-3">
              ...
              {member.teamId ? <Badge tone="accent">Team</Badge> : null}
              <select
